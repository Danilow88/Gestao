...
def create_working_ping_system():
    """Sistema de ping local FUNCIONAL que realmente executa o ping"""
    
    st.title("🧪 Sistema de Ping Local FUNCIONAL - TESTE")
    ...
    if st.button("🚀 EXECUTAR PING LOCAL FUNCIONAL", key="ping_button"):
        ...
        # Mostrar dados em formato de tabela também
        ...
        results_df = pd.DataFrame([
            {
                ...
                "Status": "ONLINE" if results[ip]["online"] else "OFFLINE",
                "Latência": f"{results[ip]['latency']}ms" if results[ip]["latency"] else "N/A",
                "Método": results[ip]["method"]
            }
            for ip, details in sample_data.items()
        ])
        
        st.dataframe(results_df, use_container_width=True)

if __name__ == "__main__":
    create_working_ping_system()
